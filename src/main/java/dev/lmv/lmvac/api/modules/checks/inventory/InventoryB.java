...
@SettingCheck(
        ...
        description = "Default InventoryMove/Click's Detection"
)
...
//               event.setCancelled(true);
                    String reason = locales.getOrDefault("1","Suspicious click in inventory during potential movement. %sp%j%sn%d")
                            .replace("%d",player.isDead()?"dead ":"")
                            .replace("%sp",player.isSprinting()?"sprinting ":"")
                            .replace("%j",player.isJumping()?"jumping ":"")
                            .replace("%sn",player.isSneaking()?"sneaking ":"");
