...
public class AimNpc extends Check implements PacketCheck, Configurable {
    private final Random random = new Random();
    public RandomMode mode = null;
    private AimMode aimMode = null;
    private AimSettings aimSettings = null;
    ...
    public void resetMode(Plugin plugin) {
    ...
    public void onPacketReceiving(PacketEvent packetEvent) {
        ...
        if (client == null || !client.isInCombat()
                || client.rotationData.cinematicRotation) return;

        List<LmvPlayer.LookInformation> looks = client.looks;
        if (looks.size() >= 2) {
