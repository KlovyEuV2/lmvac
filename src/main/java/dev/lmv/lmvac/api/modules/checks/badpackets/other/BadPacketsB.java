...
public class BadPacketsB extends Check implements PacketCheck, BukkitCheck, Configurable {
    public BadPacketsB(Plugin plugin) {
        super(plugin);
        reloadConfiguration(plugin);
    }

    public List<ShulkerProcess> processingShulkers = new ArrayList<>();
    public long lastTimeUpdate = -1;

    public double velocity_y = -999;
    ...
    public void cleanUpOlder(long maxOld) {
        long now = System.currentTimeMillis();
        processingShulkers.removeIf(k -> now - k.time > maxOld);
    }

    public boolean isShulkerNearby(Location location, double distance) {
    ...
    public void reloadConfiguration(Plugin plugin) {
        ConfigurationSection section = plugin.getConfig().getConfigurationSection("checks.badpackets.b");
        velocity_y = section.getDouble("velocity-y",-999);
    }
        ...
        if (packetType.equals(PacketType.Play.Client.POSITION)) {
            ...
            if (diffY > maxY && nearShulker) {
                ...
                Bukkit.getScheduler().runTask(plugin, () -> {
                    ...
                    player.setVelocity(new Vector(0,velocity_y,0));
                });
