...
public class InventoryE extends Check implements BukkitCheck, PacketCheck, Configurable {

    public int IMoves = 5;
    public List<PacketType> allowed = new ArrayList<>();
    ...
    public void reloadConfiguration(Plugin plugin) {
        ConfigurationSection section = plugin.getConfig().getConfigurationSection("checks.inventory.e");
        IMoves = 5;
        List<String> allowedN = new ArrayList<>();
        try {
            allowedN = section.getStringList("packets");
            IMoves = section.getInt("cancel-moves", 5);
        } catch (NullPointerException ex) {
            allowedN = new ArrayList<>();
            IMoves = 5;
        }
        if (allowedN.isEmpty()) allowedN.add("POSITION_LOOK");
