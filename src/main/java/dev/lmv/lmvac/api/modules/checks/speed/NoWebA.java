...
public class NoWebA extends Check implements BukkitCheck {
    ...
    private static final ConcurrentHashMap<UUID, List<Long>> sendFlag = new ConcurrentHashMap<>();
    private static final ConcurrentHashMap<UUID, Location> lastLocation = new ConcurrentHashMap<>();
    ...
    private void onMove(PlayerMoveEvent event) {
        ...
        if (!WebUtil.isInWeb(player)) {
            ...
            boolean fromWeb = isInWeb(from) || isInWeb(from.clone().add((double)0.0F, 1.1321, (double)0.0F));
            boolean toWeb = isInWeb(to) || isInWeb(to.clone().add((double)0.0F, 1.1321, (double)0.0F));
            if (fromWeb && toWeb) {
    ...
    private static boolean checkUnfair(Player player) {
        return player.isFlying() || player.isGliding() || player.getVelocity().getY() > 0.8 || player.getVelocity().getX() > (double)0.25F || player.getVelocity().getZ() > (double)0.25F || player.isInsideVehicle() || player.isDead() || player.isSleeping() || !player.isOnline();
    }
}
